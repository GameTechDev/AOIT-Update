...
{
#ifdef _CPUT
 float4 diffuseTexture = texture_DM.Sample( SAMPLER0, (((input.UV0)) *(1)) )


#else
float4 diffuseTexture = tex2D( SAMPLER0, (((input.UV0)) *(1)) )
#endif
;
...
{
    return (EMISSIVETMP(input)) *(0.25);
}

// -------------------------------------
float4 SPECULAR( PS_INPUT input )
{
    return (SPECULARTMP(input)) *(1);
}
