...
float4 DIFFUSE( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_DM.Sample( SAMPLER0, (((input.UV0)) *(2)) )
#else
tex2D( SAMPLER1, (((input.UV0)) *(2)) )
#endif
;
}
...
float4 SPECULARTMP( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_SM.Sample( SAMPLER0, (((input.UV0)) *(20)) )
#else
tex2D( SAMPLER3, (((input.UV0)) *(20)) )
#endif
;
}
...
float4 SPECULARITY( PS_INPUT input )
{
    return ((
#ifdef _CPUT
texture_ST.Sample( SAMPLER0, (((input.UV0)) *(20)) )
#else
tex2D( SAMPLER4, (((input.UV0)) *(20)) )
#endif
) *(128)) +(4);
}
...
float4 NORMAL( PS_INPUT input )
{
    return ((
#ifdef _CPUT
texture_NM.Sample( SAMPLER0, (((input.UV0)) *(2)) )
#else
tex2D( SAMPLER2, (((input.UV0)) *(2)) )
#endif
) *(2)) -(1);
}
