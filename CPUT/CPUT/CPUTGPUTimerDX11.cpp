...
static bool GetQueryDataHelper( ID3D11DeviceContext * context, bool loopUntilDone, ID3D11Query * query, void * data, int dataSize )
{
   ...
   do
   {
      ...
      if( attempts > 100 ) 
          Sleep(1);
      if( attempts > 1000 ) 
        { ASSERT( false, L"CPUTGPUTimerDX11.cpp - Infinite loop while doing s_immediateContext->GetData() - this shouldn't happen. " _CRT_WIDE(__FILE__) L", line: " _CRT_WIDE(_CRT_STRINGIZE(__LINE__)) ); return false; }
