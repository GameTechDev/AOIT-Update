...
CPUTResult CPUT_DX11::CreateDXContext(CPUTContextCreation ContextParams )
{
	...
	mSwapChainSampleCount = ContextParams.swapChainSampleCount;
    ...
    sd.SampleDesc.Count = mSwapChainSampleCount;  // Number of MSAA samples
    ...
    bd.ByteWidth = sizeof(CPUTInstanceConstantBuffer::CPUTModelConstantBuffer);
    bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    bd.Usage = D3D11_USAGE_DYNAMIC;
    bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    ...
    ID3D11Buffer* pGUIBuffer;
    bd.ByteWidth = sizeof(GUIConstants);
    bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    bd.Usage = D3D11_USAGE_DYNAMIC;
    bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = (CPUT_DX11::GetDevice())->CreateBuffer( &bd, NULL, &pGUIBuffer );
    ASSERT( !FAILED( hr ), _L("Error creating constant buffer.") );
    CPUTSetDebugName( pGUIBuffer, _L("Model Constant buffer") );
    name = _L("$cbGUIConstants");
    CPUTBufferDX11* pConstants = new CPUTBufferDX11(name, pGUIBuffer);
    CPUTAssetLibrary::GetAssetLibrary()->AddConstantBuffer( name, _L(""), _L(""), pConstants );
    SAFE_RELEASE(pConstants);
    SAFE_RELEASE(pGUIBuffer);
    return result;
}
...
CPUTResult CPUT_DX11::CreateAndBindDepthBuffer(int width, int height)
{
   ...
   depthBufferDesc.SampleDesc.Count = mSwapChainSampleCount; // Number of MSAA samples
    ...
    srvDesc.ViewDimension = (mSwapChainSampleCount>1)?(D3D11_SRV_DIMENSION_TEXTURE2DMS):(D3D11_SRV_DIMENSION_TEXTURE2D);
    ...
    depthStencilViewDesc.ViewDimension = (mSwapChainSampleCount>1)?(D3D11_DSV_DIMENSION_TEXTURE2DMS):(D3D11_DSV_DIMENSION_TEXTURE2D);
...
void CPUT_DX11::UpdatePerFrameConstantBuffer( CPUTRenderParameters &renderParams, double totalSeconds )
{
    if( mpPerFrameConstantBuffer )
    {
        ...
        pCb->MultiSampleCount   = float4( (float)mSwapChainSampleCount, 1.0f/(float)mSwapChainSampleCount, 0.0f, 0.0f );
...
void CPUT_DX11::InnerExecutionLoop()
{
    ...
    if(!mbShutdown)
    {
		...
		CPUTGPUTimerDX11::OnFrameStart( );
        Render(deltaSeconds);
		CPUTGPUTimerDX11::OnFrameEnd( );
...
CPUTResult CPUT_DX11::CPUTCreateWindowAndContext(const cString WindowTitle, CPUTWindowCreationParams windowParams)
{
    ...
    //
    mpTimer->StartTimer();
	CPUTGPUTimerDX11::OnDeviceAndContextCreated( mpD3dDevice, mpContext );
...
{
	CPUTGPUTimerDX11::OnDeviceAboutToBeDestroyed( );
