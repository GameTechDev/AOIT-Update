...
CPUTResult CPUTMeshDX11::CreateNativeResources(
    ...
    if(NULL!=pIndexData)
    {
		...
		mpIndexData = new char[mIndexCount * pIndexDataInfo->mElementSizeInBytes];
		memcpy(mpIndexData, pIndexData, mIndexCount * pIndexDataInfo->mElementSizeInBytes);
		// CC added ends
    }
	...
	mpVertexData = new char[mVertexCount * mVertexStride];
	memcpy(mpVertexData, pVertexData, mVertexCount * mVertexStride);
	// CC added ends
