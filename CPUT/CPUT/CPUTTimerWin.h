...
class CPUTCPUProfiler_AutoScopeProfile
{
	CPUTTimerWin &                    m_profiler;
	const bool                        m_doProfile;

public:
	CPUTCPUProfiler_AutoScopeProfile(CPUTTimerWin & profiler, bool doProfile = true) : m_profiler(profiler), m_doProfile(doProfile)
	{
		if (m_doProfile) m_profiler.StartTimer();
	}
	~CPUTCPUProfiler_AutoScopeProfile()
	{
		if (m_doProfile) m_profiler.StopTimer();
	}
};
