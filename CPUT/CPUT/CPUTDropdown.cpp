...
int CPUTDropdown::GetSelectedItemZeroBasedIndex()
{
...
{
    if( (index>-1) && (index < (int)mpListOfSelectableItems.size() ))
    {
        mSelectedItem = (index);
        //mConfirmedSelectedItemIndex = mSelectedItemIndex; // set the hard-selected item to this as well
    }
...
{
    if((0==index) || (index>(int)mpListOfSelectableItems.size()))
        return;
...
CPUTEventHandledCode CPUTDropdown::HandleMouseEvent(int x, int y, int wheel, CPUTMouseState state, CPUTEventID eventID)
{
    ...
    {
        if((eventID == CPUT_EVENT_UP) && mouseInTop)
        {
            //should update to for slightly subtle behavior on what happens here, but
            //simple for now
            //leave tray open
            return CPUT_EVENT_HANDLED;
        }
        if((eventID == CPUT_EVENT_MOVE) && mouseInTop)
        {
            //should update to for slightly subtle behavior on what happens here, but
            //simple for now
            //leave tray open
        ...
        if((eventID == CPUT_EVENT_UP) && mouseInTray) 
        {
        ...
        if(eventID == CPUT_EVENT_DOWN && mouseInTop)
        {
            mHighlightedItem = mSelectedItem;
            mControlGuiState = CPUT_DROPDOWN_GUI_MOUSE_NEUTRAL;
            return CPUT_EVENT_HANDLED;
        }
        if(eventID == CPUT_EVENT_DOWN && mouseInTray)
        {
