...
CPUTCamera &CPUTCamera::operator= (const CPUTCamera & other)
{
	mMode				= other.mMode;
    mFov				= other.mFov;                // the field of view in degrees
    mNearPlaneDistance	= other.mNearPlaneDistance;
    mFarPlaneDistance	= other.mFarPlaneDistance;
    mAspectRatio        = other.mAspectRatio; // width/height.  TODO: Support separate pixel and viewport aspect ratios
    mWidth				= other.mWidth;              // width and height for orthographic projection
    mHeight				= other.mHeight;
    memcpy(&mView, &other.mView, sizeof(float4x4));
    memcpy(&mProjection, &other.mProjection, sizeof(float4x4));
	return *this;
}


bool CPUTCamera::operator== (const CPUTCamera& rhs)
{

    if(memcmp(&mView, &rhs.mView, sizeof(float4x4)))
		return false;

    if(memcmp(&mProjection, &rhs.mProjection, sizeof(float4x4)))
		return false;
	...
	if(mNearPlaneDistance!=rhs.mNearPlaneDistance)
		return false;

	if(mFarPlaneDistance!=rhs.mFarPlaneDistance)
		return false;

	if(mAspectRatio!=rhs.mAspectRatio)
		return false;
