...
CPUTFont *CPUTFont::CreateFont(cString FontName, cString AbsolutePathAndFilename)
{
    ...
    CPUTFont *pFont;
    pFont = CPUTFont::LoadBMFont( pName, pPath );
    delete pName;
    delete pPath;
...
//
CPUTFont *CPUTFont::LoadBMFont(const std::string& pPath, const std::string& pFileName)
{
    UINT      fileSize   = 0;
    uint8_t  *pData      = NULL;
...
void CPUTFont::LayoutText(CPUTGUIVertex *pVtxBuffer, int *pWidth, int *pHeight, const std::string& text, int tlx, int tly)
{
    ...
    for (uint32_t j = 0, index = 0; j < text.length(); j++) {
        ...
        if(pVtxBuffer)
        {
            pVtxBuffer[index+0].Pos   = float3( (float)(x + 0 + pChar->xoffset + kernAmount), (float)(y + 0 + pChar->yoffset), 1.0f);
            pVtxBuffer[index+0].UV    = float2(pChar->x / texWidth, pChar->y / texHeight);
            pVtxBuffer[index+0].Color = redColor;

            pVtxBuffer[index+1].Pos   = float3( (float)(x + pChar->width + pChar->xoffset + kernAmount), float(y + 0 + pChar->yoffset), 1.0f);
            pVtxBuffer[index+1].UV    = float2((pChar->x + pChar->width) / texWidth, pChar->y / texHeight);
            pVtxBuffer[index+1].Color = redColor;

            pVtxBuffer[index+2].Pos   = float3( (float)(x + 0.0f + pChar->xoffset + kernAmount), (float)(y + pChar->height + pChar->yoffset), 1.0f);
            pVtxBuffer[index+2].UV    = float2(pChar->x / texWidth, (pChar->y + pChar->height) / texHeight);
            pVtxBuffer[index+2].Color = redColor;

            pVtxBuffer[index+3].Pos   = float3( (float)(x + pChar->width + pChar->xoffset + kernAmount), (float)(y + 0 + pChar->yoffset), 1.0f);
            pVtxBuffer[index+3].UV    = float2((pChar->x + pChar->width) / texWidth, pChar->y / texHeight);
            pVtxBuffer[index+3].Color = redColor;

            pVtxBuffer[index+4].Pos   = float3( (float)(x + pChar->width + pChar->xoffset + kernAmount), (float)(y + pChar->height + pChar->yoffset), 1.0f);
            ...
            pVtxBuffer[index+5].Pos   = float3( (float)(x + 0 + pChar->xoffset + kernAmount), (float)(y + pChar->height + pChar->yoffset), 1.0f);
