...
#define OUTPUT_BINDING_DEBUG_INFO(x) //DEBUG_PRINT(x)
...
void CPUTMaterialEffectDX11::SetRenderStates( CPUTRenderParameters &renderParams )
{
    ...
    if( mPixelShaderParameters.mUAVCount )
    {
        ...
        UINT rtvCount;
        for( rtvCount=D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT; rtvCount>0; rtvCount-- )
        {
            if( 0 != pRTVs[rtvCount-1] ) break;
        }
        ...
        pContext->OMSetRenderTargetsAndUnorderedAccessViews(
            ...
            pDSV,
            rtvCount,
            CPUT_MATERIAL_MAX_UAV_SLOTS,
            &mPixelShaderParameters.mppBindUAVs[0],
            NULL
        );
