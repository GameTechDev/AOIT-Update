...
void CPUTGUIElement::SetText(cString string)
{
    ...
    if(mpFont && mpTextMaterial)
    {
        ...
        size_t numCharacters = string.size();
		size_t numVertices = numCharacters * 6;
        ...
        char* tempString = ws2s(mText);
        mpFont->LayoutText(NULL, &mTextWidth, &mTextHeight, tempString, 0, 0);
        mTextX = (mWidth-mTextWidth)/2;
        mTextY = (mHeight-mTextHeight)/2;
        mpFont->LayoutText(pVB, &mTextWidth, &mTextHeight, tempString, 0, 0);
        mpTextMesh->CreateNativeResources(NULL, 1, 3, pGUIVertex, (int)numVertices, pVB, NULL, 0, NULL);
...
CPUTEventHandledCode SendEvent(int eventID, CPUTCallbackHandler* pHandler,CPUTGUIElement* pGUI)
{
    if(eventID > 0)
    {
        pHandler->HandleGUIElementEvent((CPUTEventID)eventID, NULL, pGUI);
