...
// Handle mouse events
//--------------------------------------------------------------------------------
CPUTEventHandledCode CPUTButton::HandleMouseEvent(int x, int y, int wheel, CPUTMouseState state, CPUTEventID eventID)
{
...
void CPUTButton::DrawIntoBuffer(CPUTGUIVertex *pVertexBufferMirror, UINT *pInsertIndex, UINT pMaxBufferSize, CPUTGUIVertex *pTextVertexBufferMirror, UINT *pTextInsertIndex, UINT MaxTextVertexBufferSize)
{
    ...
    int copySize = sizeof(CPUTGUIVertex)*VertexCopyCount;
    ...
    case CPUT_CONTROL_ACTIVE:
        ...
        if(CPUT_BUTTON_NEUTRAL == mButtonState)
        {
            memcpy(&pVertexBufferMirror[*pInsertIndex], mpMirrorBufferActive, copySize);
        }        

        // copy the pressed button into the stream
        if(CPUT_BUTTON_PRESSED == mButtonState)
        {
            memcpy(&pVertexBufferMirror[*pInsertIndex], mpMirrorBufferPressed, copySize);
        }
        
        break;
    case CPUT_CONTROL_INACTIVE:
        // copy the inactive button into the stream
        memcpy(&pVertexBufferMirror[*pInsertIndex], mpMirrorBufferDisabled, copySize);
        break;
