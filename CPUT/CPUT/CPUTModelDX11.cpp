...
void CPUTModelDX11::Render(CPUTRenderParameters &renderParams, int materialIndex)
{
    ...
    if( renderParams.mDrawModels && !pParams->mRenderOnlyVisibleModels || !pCamera || pCamera->mFrustum.IsVisible( mBoundingBoxCenterWorldSpace, mBoundingBoxHalfWorldSpace ) )
    {
        ...
        for(UINT ii=0; ii<mMeshCount; ii++)
        {
            ...
            CPUTMaterialEffect *pOriginalMaterialEffect = (CPUTMaterialEffect*)(mpMaterialEffect[ii][OriginalMaterialIndex]);

			mDrawModelCallBackFunc(this, renderParams, mpMesh[ii],pMaterialEffect,pOriginalMaterialEffect,mpInputLayout[ii][finalMaterialIndex]);
        }
    }
...
CPUTResult CPUTModelDX11::LoadModel(CPUTConfigBlock *pBlock, int *pParentID, CPUTModel *pMasterModel, int numSystemMaterials, cString *pSystemMaterialNames)
{
    ...
    *pParentID = pBlock->GetValueByName(_L("parent"))->ValueAsInt();
    ...
    for(UINT ii=0; ii<mMeshCount; ii++)
    {
		...
		{
			shadowMaterialName = pAssetLibrary->GetMaterialEffectPath(	shadowMaterialName , false);
		}
...
void CPUTModelDX11::DrawBoundingBox(CPUTRenderParameters &renderParams)
{
 ...
 //   CPUTMaterialDX11 *pMaterial = (CPUTMaterialDX11*)(mpMaterial[0][index]);
	...
	UINT finalMaterialIndex = GetMaterialIndex(0, CPUT_MATERIAL_INDEX_BOUNDING_BOX);
	ASSERT(finalMaterialIndex < mpLayoutCount[0], _L("material index out of range."));
	CPUTMaterialEffect *pMaterialEffect = (CPUTMaterialEffect*)(mpMaterialEffect[0][finalMaterialIndex]);
	pMaterialEffect->SetRenderStates(renderParams);

    ((CPUTMeshDX11*)mpBoundingBoxMesh)->Draw(renderParams, mpInputLayout[0][finalMaterialIndex]);
}
